module heisenberg 
    ...
    subroutine H_XXX_filling(N_spin, J_spin, i, j, H_full_ij)
        implicit none
        integer (8), intent(in) :: N_spin, i, j 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine MPI_workload_balancing(Sz_subspace_size, size_of_cluster, mpi_rows_collected)
        use omp_lib
        implicit none
        include 'mpif.h'
        integer(8), intent(in) :: Sz_subspace_size
        integer, intent(in) :: size_of_cluster
        integer(8), allocatable, intent(out) :: mpi_rows_collected(:,:)
        integer(8) :: N, MPI, perfect_N, elements_left, ind, ind2, mpi_row_start, ind_mpi, rows_per_mpi, el_in_row, el_to_be_burn, total_rows_burn, sum_check, no_of_els_to_calculate
        ...
        if (  sum_check .EQ. (N*(N+1))/2 ) then
            write(*,*) 'rows checked, elements checked, number of elements in the Hamiltonian is equal to expected number of elements'
            write(*,*) 'sum_check of elements .EQ. (N/2*(N+1))', sum_check, (N*(N+1))/2
        else
    ...
    subroutine Hamiltonian_MPI_OpenMP_pfeast(hash_Sz, Sz_subspace_size,  N_spin, J_spin, size_ia, size_ja, size_val, number_of_rows_local_MPI)        
        ...
        write(*,*) '----------  START of CSR vector filling -------------------'
        write(*,*) ' '
        ...
        if (end_row .GT. Sz_subspace_size) then
            write(*,*) 'end_row > Sz_subspace_size'
            error stop
        endif

        ! assign rows to each mpi process
        write(*,*) ' Entering MPI_workload_balancing'
        call MPI_workload_balancing(Sz_subspace_size, size_of_cluster, mpi_rows_collected)
        write(*,*) ' Exiting MPI_workload_balancing'
        write(*,*) ' '
        ...
        open (unit=10, file='rows_element_local'//trim(loc_id_info)//'.dat', recl=512)
        
        !write to file for testing size of mpi_rows_collected (can be deleted later)
        do ind = 1, size(mpi_rows_collected,1)        
            write(10,*) mpi_rows_collected(ind,1), mpi_rows_collected(ind,2)        
        end do
        
        !$OMP PARALLEL
        omp_id = omp_get_num_threads()
        !$OMP END PARALLEL

        window_size = omp_id ! should be omp_procs * x
        write(*,*) 'Number of OpenMP threads check vs windows_size: ', omp_id, window_size
        ...
        !ind = 1
        write(*,*) 'row_start, row_end per this local process', local_process_ID, row_start, row_end
        no_of_ja_val_el = 0
        ia_temp_ind = 0
        temp_ind =1
        ...
        !enable size of final ia, ja and val_arr for next function to know what to read from the disk
        ...
        write(*,*) '----------  END of CSR vector filling -------------------'
        write(*,*) ' '
        ...
        end subroutine Hamiltonian_pfeast_diagonalisation
