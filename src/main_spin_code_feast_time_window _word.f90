...
program spin_code
    ...
    ! tworzenie bazy i s_z dzia?a
    ! macierz permutacji H dzia?a 
    ! test dla obrotu H n = 4 dzia?a 

    ! do zrobienia
    ! zapis eigenvectora do pliku - podzia? na podpliki 

    ! TO DO LATER
    ! Zacznij ciac odpowiednie wektory do S_z , pytanie czy to w fortranie czy nie? 
    ! chyba w fotranie + diagonalizacja za pomoc? feasta blockowych macierzy 
    ! w pythonie robisz to tnac pe?n? macierz H
    ! pytanie do Paw?a: jak bra? wycinek macierzy w H blockowym do feasta (numer eigenvalues i okno energetyczne)
